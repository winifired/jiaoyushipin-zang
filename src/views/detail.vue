<template>
  <div class="detail safeBottom120" v-if="info">
    <div class="video-img">
      ...
      <div class="tip flex area-center">
        <!--免费课程也可以直接观看，购买后可观看 -->
    ...
    <div class="class-content">
      <div class="flex column-bwn">
        ...
        <div class="collectStatus" @click="toggleCollect">
          <img src="../assets/star-act.png" alt class="icon44" v-if="userid && info.isCollect == 1" />
          <img src="../assets/star.png" alt class="icon44" v-if="!userid || info.isCollect != 1" />
          <p class="f26 c777">
            <span>{{$t('sc')}}</span>
            {{ collectNum }}
          </p>
      ...
      <div class="class-msg">
        ...
        <div class="class-detail" v-if="actived == 3">
          <div class="detail-eva">
            ...
            >
              <div @click="toEva" class="f26 c333 bgf9d toLook">{{$t('woyaopingjia')}}</div>
            </div>
            <div class="evas">
  ...
  <div class="fixedbottom flex area-center safeBottom" v-if="info&&userinfo">
    ...
    <div
      class="button-bottom bgf9d"
      @click="openpay"
      v-if="info.mode == 1||(info.mode == 2 && userinfo.level == 1)"
    >
  ...
  <div class="fixedbottom flex area-center safeBottom" v-if="info&&!userinfo">
    <!-- 免费可直接观看 -->
    <!-- 未购买 -->
    <div class="button-bottom bgf9d" @click="openpay" v-if="info.mode == 2 && info.isBuy != 1">
      <span class="c333 f30 FZLTTHJWF marginr">¥{{ info.price }}</span>
      <span class="c333 f32">{{$t('lijidingyue')}}</span>
      <!-- 立即订阅加入学习 -->
    </div>
...
import { Rate, Sticky, List, Swipe, SwipeItem, Dialog } from "vant";
  ...
  setup() {
    ...
    function getData() {
      ...
      proxy.$post(url, data).then((res) => {
        if (res.code == 0) {
          res.data.carousel = res.data.carousel
            ? res.data.carousel.split(",")
            : [];
          state.info = res.data;
      ...
      // 收藏数量
      proxy
        .$post(proxy.Apis.countByCourseId, {
          courseId: route.params.id,
        })
        .then((res) => {
    ...
    function getlistCatalogueByCourseId() {
      // 获取系列课程的目录
      proxy
        .$post(proxy.Apis.listCatalogueByCourseId, {
          courseId: route.params.id,
    ...
    function getComment() {
      // 获取评论列表
      proxy
        .$post(proxy.Apis.listCommentByCourseId, {
          courseId: route.params.id,
          pageNum: 1,
          pageSize: 5,
        })
    ...
    function toEva() {
      if (!state.userid) {
        ...
        if (
          state.info.mode == 1 ||
          (state.info.mode == 2 &&
            (state.userinfo.level == 1 || state.info.isBuy == 1))
        ) {
          router.push({
            path: "/eva_use",
            query: {
    ...
    function showDialog() {
      Dialog.confirm({
        ...
        title: t("saw"),
        message: t("saw2"),
        confirmButtonText: t("saw4"),
        confirmButtonColor: "#007AFF",
        cancelButtonText: t("saw3"),
